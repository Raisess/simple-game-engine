...
int main(int argc, char** argv) {
  ...
  auto* some_component = new Engine::ScreenComponent(window, 0, 0, 100, 100, false);
  ...
  const auto callback = [window, some_component]() -> void {
    ...
    if (key[SDL_SCANCODE_RIGHT]) {
      some_component->update_pos(some_component_pos.x + 10, some_component_pos.y);
    } else if (key[SDL_SCANCODE_LEFT]) {
      some_component->update_pos(some_component_pos.x - 10, some_component_pos.y);
    } else if (key[SDL_SCANCODE_UP]) {
      some_component->update_pos(some_component_pos.x, some_component_pos.y - 10);
